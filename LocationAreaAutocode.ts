function locationCodeFromPlace(loc: string): number | null {
  ...
  if ((loc.includes("farm") && loc.includes("house")) && (!loc.includes("other"))) {
    ...
    return 91;
  } else if ((loc.includes("restaurant")) || (loc.includes("snack bar")) || (loc.includes("coffee shop"))) {
    return 92;
    ...
    return 93;
  } else if ((loc.includes("airport")) || (loc.includes("bus station")) || (loc.includes("railway station")) || (loc.includes("subway station")) || (loc.includes("ferry terminal"))) {
    return 94;
    ...
    return 109;
  } else if ((loc.includes("bush")) || (loc.includes("remote")) || (loc.includes("snowmobile trail")) || (loc.includes("swamp"))) {
    return 141;
    ...
    return 65;
  } else if (loc.includes("fitness training club") || loc.includes("fitness facility") || loc.includes("martial arts studio") || loc.includes("dance studio") || loc.includes("gymnastics club") || loc.includes("ballet")) {
    return 66;
    ...
    return 67;
  } else if (loc.includes("golf course") || loc.includes("equestrian centre") || loc.includes("rodeo grounds")) {
    return 68;
    ...
    return 71;
  } else if (loc.includes("bowling alley") || loc.includes("pool hall") || loc.includes("arcade") || loc.includes("bingo hall") || loc.includes("cosmic adventures") || loc.includes("boys and girls club")) {
    return 79;
...
function main(workbook: ExcelScript.Workbook) {
  ...
  const placelocColumnIndex = 15; // Replace with the index of the column you want to process
  const locColumnIndex = 13;
  processLocColumn(sheet, placelocColumnIndex, locColumnIndex);
 
  const placeareaColumnIndex = 16; // Replace with the index of the column you want to process
  const areaColumnIndex = 14;
